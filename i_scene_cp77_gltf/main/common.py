...
def CreateShaderNodeNormalMap(curMat,path = None, x = 0, y = 0, name = None,image_format = 'png', nonCol = True, uvMapNode = None):
    nMap = curMat.nodes.new("ShaderNodeNormalMap")
    nMap.location = (x,y)
    if name is not None:
        nMap.label = name + 'Normal Map'
    ...
    if path is not None:
        ...
        # map the uv coords if we have them
        if uvMapNode is not None:
            curMat.links.new(uvMapNode.outputs[0],ImgNode.inputs[0])
